    ...
    <div style={{ display: "grid", gap: 16 }}>
      <Card>
        ...
        <Muted style={{ fontSize: 14 }}>
          {done} of {scheduled.length} habits complete
        </Muted>
        ...
        <div style={{ marginTop: 20, display: "grid", gap: 8 }}>
          ...
          {scheduled.map((h) => {
            ...
            return (
              <div
                key={h.id}
                style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "8px 4px" }}
              >
                <button
                  onClick={() => toggle(h.id)}
...
/* ============================ GRATITUDE ============================ */
/* ============================ THOUGHTS ============================ */
function ThoughtsTab({ store, onNavigate }: { store: Store; onNavigate?: (tab: Tab) => void }) {
  ...
  const activeThoughts = store.state.thoughts
    .filter((t) => !t.archived)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  const archivedLast5 = store.state.thoughts
    .filter((t) => t.archived)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt))
    .slice(0, 5);
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 16, alignItems: "start" }}>
      <div style={{ display: "grid", gap: 16 }}>
        ...
        {viewingThought ? (
          <Card>
            ...
            >
              <SectionTitle>Archived Note</SectionTitle>
              <Button variant="ghost" onClick={() => setViewingId(null)}>
                Close
              </Button>
            </div>
            <div>
              {viewingThought.header && (
              ...
              {viewingThought.body && (
                <div
                  style={{ fontSize: 14, whiteSpace: "pre-wrap", lineHeight: 1.5, marginBottom: 8 }}
                >
                  {viewingThought.body}
                </div>
              )}
              <Muted style={{ fontSize: 11, display: "block", marginBottom: 10 }}>
                {formatTs(viewingThought.createdAt)}
                {viewingThought.updatedAt &&
                  viewingThought.updatedAt !== viewingThought.createdAt &&
                  ` · edited ${formatTs(viewingThought.updatedAt)}`}
              </Muted>
              <div style={{ display: "flex", gap: 8 }}>
                <Button onClick={() => unarchive(viewingThought.id)}>Unarchive</Button>
                <Button variant="danger" onClick={() => del(viewingThought.id)}>
                  Delete
                </Button>
              </div>
            </div>
          ...
          <Card>
            <SectionTitle>Notes ({activeThoughts.length})</SectionTitle>
            {activeThoughts.length === 0 && <Muted>No thoughts yet.</Muted>}
      ...
      <div style={{ display: "grid", gap: 16 }}>
        <Card>
          ...
          {archivedLast5.length === 0 && <Muted>No archived notes.</Muted>}
          <div style={{ display: "grid", gap: 8 }}>
            {archivedLast5.map((t) => (
              <button
                key={t.id}
              ...
              >
                ...
                <Muted style={{ fontSize: 11 }}>{formatTs(t.createdAt)}</Muted>
              </button>
            ))}
          </div>
          {store.state.thoughts.filter((t) => t.archived).length > 5 && (
            <div style={{ marginTop: 8 }}>
              <Button variant="ghost" onClick={() => onNavigate?.("thot-archive")}>
                View all in Thot-Archive →
              </Button>
            </div>
          )}
        </Card>
...
/* ============================ THOT-ARCHIVE ============================ */
function ThotArchiveTab({ store }: { store: Store }) {
  ...
  const archived = store.state.thoughts
    .filter((t) => t.archived)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <Card>
        <SectionTitle>Thot-Archive ({archived.length})</SectionTitle>
        {archived.length === 0 && <Muted>No archived notes.</Muted>}
        <div style={{ display: "grid", gap: 4 }}>
          {archived.map((t) => (
            <div key={t.id} style={{ borderBottom: `1px solid ${COLORS.border}` }}>
              ...
              >
                <span style={{ fontSize: 12, color: COLORS.sub, width: 16, flexShrink: 0 }}>
                  {expandedId === t.id ? "▼" : "▶"}
                </span>
                <span
                  style={{
                ...
                <Muted style={{ fontSize: 12, flexShrink: 0 }}>{formatTs(t.createdAt)}</Muted>
              </button>
              {expandedId === t.id && (
                <div style={{ padding: "0 8px 12px 32px" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <Button onClick={() => unarchive(t.id)}>Unarchive</Button>
                    <Button variant="danger" onClick={() => del(t.id)}>
                      Delete
                    </Button>
                  </div>
...
function SleepWeekRows({ store, days }: { store: Store; days: Date[] }) {
  ...
  const rows: Array<{
    label: string;
    get: (log: import("@/lib/habit-data").SleepLog | undefined) => string;
  }> = [
...
function MonthTab({ store }: { store: Store }) {
  ...
  return (
    <Card>
      ...
      >
        <Button variant="secondary" onClick={() => setAnchor(new Date(year, month - 1, 1))}>
          ← Prev
        </Button>
        <div style={{ fontWeight: 600 }}>
...
function CycleTab({ store }: { store: Store }) {
  ...
  const logPeriod = () => {
    ...
    store.update((s) => {
      ...
      return {
        ...s,
        cycle: { ...s.cycle, periods: [...s.cycle.periods, { start: date, logged: true }] },
      };
    });
  };
  const remove = (start: string) => {
    store.update((s) => ({
      ...s,
      cycle: { ...s.cycle, periods: s.cycle.periods.filter((p) => p.start !== start) },
    }));
  };
          ...
          {[...c.periods]
            ...
            .map((p) => (
              ...
              >
                ...
                <Button variant="danger" onClick={() => remove(p.start)}>
                  <IconTrash />
                </Button>
              </div>
            ))}
...
function TodosTab({ store }: { store: Store }) {
  ...
  const clearCompleted = () =>
    store.update((s) => ({ ...s, todos: s.todos.filter((t) => !t.done) }));
  const saveEdit = (id: string) => {
    store.update((s) => ({
      ...s,
      todos: s.todos.map((t) => (t.id === id ? { ...t, text: editText } : t)),
    }));
    setEditingId(null);
  };
...
function BuysTab({ store }: { store: Store }) {
  ...
  const toggle = (id: string) =>
    store.update((s) => ({
      ...
      buys: s.buys.map((b) =>
        b.id === id
          ? { ...b, done: !b.done, completedAt: !b.done ? new Date().toISOString() : null }
          : b,
      ),
    }));
  const del = (id: string) =>
    store.update((s) => ({ ...s, buys: s.buys.filter((b) => b.id !== id) }));
  const clearCompleted = () => store.update((s) => ({ ...s, buys: s.buys.filter((b) => !b.done) }));
  const saveEdit = (id: string) => {
    store.update((s) => ({
      ...s,
      buys: s.buys.map((b) =>
        b.id === id ? { ...b, text: editText, price: Number(editPrice) || 0 } : b,
      ),
    }));
    setEditingId(null);
  };
  ...
  const completed = store.state.buys
    .filter((b) => b.done)
    .sort((a, b) => (b.completedAt || "").localeCompare(a.completedAt || ""));
            ...
            >
              ...
              {editingId === b.id ? (
                ...
                <>
                  ...
                  <Button variant="danger" onClick={() => del(b.id)}>
                    <IconTrash />
                  </Button>
                </>
              )}
...
function GroceryTab({ store }: { store: Store }) {
  ...
  const clearCompleted = () =>
    store.update((s) => ({ ...s, groceries: s.groceries.filter((g) => !g.done) }));
  const saveEdit = (id: string) => {
    store.update((s) => ({
      ...s,
      groceries: s.groceries.map((g) =>
  ...
  const completed = store.state.groceries
    .filter((g) => g.done)
    .sort((a, b) => (b.completedAt || "").localeCompare(a.completedAt || ""));
...
function AnalysisTab({ store }: { store: Store }) {
  ...
  const cyclePhases: Record<Phase, { total: number; done: number }> = {
    menses: { total: 0, done: 0 },
    follicular: { total: 0, done: 0 },
    fertile: { total: 0, done: 0 },
    luteal: { total: 0, done: 0 },
  };
  ...
  // Todos throughput
  const todosInRange = store.state.todos.filter(
    (t) => t.createdAt >= todayStr(start) && t.createdAt <= todayStr(end),
  );
  const todosCompleted = store.state.todos.filter(
    (t) => t.completedAt && t.completedAt >= todayStr(start) && t.completedAt <= todayStr(end),
  );
  const avgDays = todosCompleted.length
    ? Math.round(
        (todosCompleted.reduce(
          (s, t) =>
            s +
            (t.completedAt
              ? Math.max(
  ...
  // Buys spend
  const buysInRange = store.state.buys.filter(
    (b) =>
      b.done && b.completedAt && b.completedAt >= todayStr(start) && b.completedAt <= todayStr(end),
  );
  ...
  const exportCsv = () => {
    ...
    for (const t of todosInRange)
      rows.push(
        `todo,${t.createdAt},"${escapeCsv(t.text)}",${t.done ? 1 : 0},${t.completedAt || ""}`,
      );
    for (const b of store.state.buys.filter(
      (x) => x.createdAt >= todayStr(start) && x.createdAt <= todayStr(end),
    )) {
    ...
    <div style={{ display: "grid", gap: 16 }}>
      <Card>
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <Button variant="secondary" onClick={() => stepAnchor(-1)}>
              ←
            </Button>
      ...
      <Card>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
          {(["menses", "follicular", "fertile", "luteal"] as Phase[]).map((p) => {
            const c = PHASE_COLORS[p];
            const pct = cyclePhases[p].total
              ? Math.round((cyclePhases[p].done / cyclePhases[p].total) * 100)
              : 0;
            return (
            ...
            >
              {editingId === h.id ? (
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        opacity: idx === habits.length - 1 ? 0.4 : 1,
                      }}
                    >
                      <IconArrowDown />
                    </button>
                  ...
                  <Button variant="danger" onClick={() => del(h.id)}>
                    <IconTrash />
                  </Button>
                </div>
              )}
...
function useReminderNotifier(store: Store) {
  useEffect(() => {
    ...
    const setFired = (v: Record<string, boolean>) =>
      window.localStorage.setItem(firedKey, JSON.stringify(v));
...
function GratitudeTab({ store }: { store: Store }) {
  ...
  const gratitudeLog = store.state.gratitude[date];
  ...
  const [items, setItems] = useState<string[]>(
    gratitudeLog?.entries?.length ? gratitudeLog.entries.map((e) => e.text) : ["", "", ""],
  );
  const [messageToUniverse, setMessageToUniverse] = useState(gratitudeLog?.messageToUniverse ?? "");
  const [saved, setSaved] = useState(false);

  useEffect(() => {
    const gl = store.state.gratitude[date];
    setMood(gl?.mood ?? null);
    setEmotions(gl?.emotions ?? []);
    setItems(gl?.entries?.length ? gl.entries.map((e) => e.text) : ["", "", ""]);
    setMessageToUniverse(gl?.messageToUniverse ?? "");
    setSaved(false);
  }, [date, store.state.gratitude]);
    ...
    store.update((s) => ({
      ...
      gratitude: {
        ...
        [date]: {
          mood,
          emotions,
          entries: cleaned.map((text) => ({ id: uid(), text })),
          messageToUniverse,
  ...
  const clearDay = () => {
    store.update((s) => {
      const next = { ...s.gratitude };
      delete next[date];
      return { ...s, gratitude: next };
    });
    setMood(null);
    setEmotions([]);
    setItems(["", "", ""]);
    setMessageToUniverse("");
  };

  const history = Object.entries(store.state.gratitude)
    .map(([d, log]) => ({ date: d, ...log }))
    .sort((a, b) => b.date.localeCompare(a.date))
    .slice(0, 30);

  const isToday = date === todayStr();
  const vd = new Date(date + "T00:00:00");
  const MONTHS = [
    "Jan",
    "Feb",
    "Mar",
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      <Card>
        ...
        <div style={{ height: 1, background: COLORS.border, margin: "20px 0", opacity: 0.4 }} />
        ...
        <Muted style={{ fontSize: 13, marginBottom: 8 }}>I am grateful for…</Muted>
        <div style={{ display: "grid", gap: 8 }}>
          {items.map((val, i) => (
        ...
        >
          <Button onClick={save}>Save</Button>
          {store.state.gratitude[date] && (
            <Button variant="danger" onClick={clearDay}>
              Clear day
            </Button>
          )}
          {saved && <Muted style={{ fontSize: 12, color: COLORS.green }}>Saved</Muted>}
          {gratitudeLog?.updatedAt && !saved && (
            <Muted style={{ fontSize: 12 }}>Last updated {formatTs(gratitudeLog.updatedAt)}</Muted>
          )}
        </div>
      </Card>
      ...
      <Card>
        ...
        {history.length === 0 ? (
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {history.map((g) => {
              const moodDef = moods.find((m) => m.value === g.mood);
              return (
                <div
                  key={g.date}
                  style={{
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      ...
                      {g.emotions?.length > 0 && (
                        <div style={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
                          {g.emotions.slice(0, 3).map((em: string) => (
                            <span
                              key={em}
                              style={{
                                fontSize: 10,
                          ...
                          {g.emotions.length > 3 && (
                            <Muted style={{ fontSize: 10 }}>+{g.emotions.length - 3}</Muted>
                          )}
                        </div>
                      )}
                    </div>
                    <Muted style={{ fontSize: 11 }}>
                      {g.entries?.length ?? 0} item
                      {(g.entries?.length ?? 0) === 1 ? "" : "s"}
                    </Muted>
                  </div>
                  {g.entries?.length > 0 && (
                    <ul style={{ margin: 0, paddingLeft: 20, fontSize: 13, color: COLORS.text }}>
                      {g.entries.map((it: { id: string; text: string }, i: number) => (
                        <li key={it.id || i}>{it.text}</li>
                      ))}
                    </ul>
                  )}
                </div>
